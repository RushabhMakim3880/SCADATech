...
use Modules\Backend\PlcMaster\Models\PlcTagGroupMasterModel;


use CodeIgniter\API\ResponseTrait;

class PlcTagGroupMaster extends ApiBaseController
{
    use ResponseTrait;


    protected $PlcTagGroupMasterModel;

    public function __construct()
    {
        $this->PlcTagGroupMasterModel = new PlcTagGroupMasterModel();
    }

    public function save($tagGroupId = 0)
    {

        $tagGroupId = (int)getKey($tagGroupId, "");

        if ($tagGroupId > 0) {
            if (!UserPermissionLib::userCanDo("PlcTagGroupMaster", 'edit')) {
                return $this->failForbidden('Insufficient permissions');
            }
        } else {
            if (!UserPermissionLib::userCanDo("PlcTagGroupMaster", 'add')) {
                return $this->failForbidden('Insufficient permissions');
            }
        }
        ...
        $jsonInput['tagGroupId'] = $tagGroupId;
        ...
        if ($tagGroupId > 0) {
            ...
            if (!$this->PlcTagGroupMasterModel->update($tagGroupId, $jsonInput)) {
            ...
            $tagGroupId = $this->PlcTagGroupMasterModel->insert($jsonInput);

            if (!$tagGroupId) {
                return $this->fail('Failed to Save', 500);
            }

            assignSerialNumber($this->user->tenantId, "plcTagGroupMaster", "tagGroupId", $tagGroupId);
        }

        $data = $this->PlcTagGroupMasterModel->find($tagGroupId);

        return $this->respondCreated(['status' => true, 'message' => $successMsg, 'data' => $data]);
    }

    public function get($tagGroupId)
    {
        $tagGroupId = (int)getKey($tagGroupId, "");

        if (!UserPermissionLib::userCanDo("PlcTagGroupMaster", 'view')) {
            return $this->failForbidden('Insufficient permissions');
        }

        $details = $this->PlcTagGroupMasterModel->find($tagGroupId);

        if (!$details) {
            return $this->failNotFound('data not found');
        }

        $details->uid = setKey($details->tagGroupId, "");
    ...
    public function getDataTableColumns($module = "")
    {
        ...
        $defaultColumns['plcTagGroupMaster_tagGroupId'] = ['title' => 'Sr.', 'visible' => true, 'orderable' => true, 'searchable' => false, 'visibleControl' => false];
        $defaultColumns['plcMaster_plcName'] = ['title' => 'Plc Name', 'visible' => true, 'orderable' => true, 'searchable' => false, 'visibleControl' => true];
        $defaultColumns['plcTagGroupMaster_groupName'] = ['title' => 'Group Name', 'visible' => true, 'orderable' => true, 'searchable' => true, 'visibleControl' => true];
        $defaultColumns['plcTagGroupMaster_description'] = ['title' => 'Description', 'visible' => true, 'orderable' => true, 'searchable' => true, 'visibleControl' => true];
        $defaultColumns['plcTagGroupMaster_createdAt'] = ['title' => 'Created At', 'visible' => true, 'orderable' => true, 'searchable' => false, 'visibleControl' => true];
        ...
        $defaultColumns['plcTagGroupMaster_createdAt']['filterType'] = 'date';
        $defaultColumns['plcTagGroupMaster_createdAt']['filterOptions'] = dateFilterOptions('past');
         ...
         *****************************************/
        $configData['defaultOrderColumn'] = 'plcTagGroupMaster_tagGroupId';
    ...
    public function getDataTableData()
    {
         ...
         *****************************************/
        $select = ["plcTagGroupMaster.serialNo as plcTagGroupMaster_serialNo"]; // Add the fields you need compulsary irrespective of the user settings, or leave empty []

        $where = ["1"];
        $where[] = "plcTagGroupMaster.tenantId = '" . $this->user->tenantId . "'";
        ...
        $dbTable = 'plcTagGroupMaster LEFT JOIN plcMaster plcMaster ON plcMaster.plcId = plcTagGroupMaster.plcId 
LEFT JOIN tenantMaster tenantMaster ON tenantMaster.tenantId = plcTagGroupMaster.tenantId 
';
        ...
        foreach ($data as $k => &$row) {
            ...
            $primaryKey = $row->plcTagGroupMaster_tagGroupId;
            ...
            if (!$isDownload) {
                ...
                if (UserPermissionLib::userCanDo("PlcTagGroupMaster", 'edit')) {
                    $editButton = "<a href='#'class='btn btn-xs btn-warning  ms-2 apiPopup'
                      data-size='xl'data-title='Edit Plc Tag'
                      data-endpoint='" . base_url("PlcTagGroupMaster/addPlcTag/" . setKey($primaryKey, "")) . "'
                      title='Edit Plc Tag'>
                      <i class='fa fa-pencil-alt'></i>
                   </a>";
                }

                $row->plcTagGroupMaster_tagGroupId = $row->plcTagGroupMaster_serialNo . ' ' . $editButton;
            ...
            $row->plcTagGroupMaster_createdAt = myDateTimeFormat($row->plcTagGroupMaster_createdAt);
            $row->plcTagGroupMaster_description = popoverText($row->plcTagGroupMaster_description);
    ...
    {
        $tagGroupData = $this->db->query("SELECT TG.tagGroupId as id, TG.groupName AS `name` FROM plcTagGroupMaster TG WHERE  TG.tenantId = " . $this->user->tenantId . "")->getResult();

        foreach ($tagGroupData as &$type) {
